"""QaraamiGen inference service — MusicGen-small + LoRA adapters, served locally.
...
BASE_MODEL = os.environ.get("MUSICGEN_BASE_MODEL", "facebook/musicgen-small")
...
def provenance(adapter: str) -> str:
    if adapter == "base":
        return f"Generated by {BASE_MODEL} with no adapter (base model). {RIGHTS_LINE}"
    info = ADAPTERS[adapter]
    return (
        f"Generated by {BASE_MODEL} with the '{adapter}' LoRA adapter, fine-tuned on {info['corpus']} "
...
# ── model engine (single worker thread) ────────────────────────────────────────
@dataclass
class Engine:
    model: object = None
    ...
    def load(self) -> None:
        ...
        base = MusicgenForConditionalGeneration.from_pretrained(BASE_MODEL, torch_dtype=torch.float32)
        ...
        self.model = model
        log.info("model loaded", extra={"detail": {"device": DEVICE, "adapters": self.loaded_adapters, "seconds": round(time.time() - t0, 1)}})
...
class GenerateRequest(BaseModel):
    ...
    adapter: Literal["base", "oud", "harvard_raw"] = "oud"
...
async def health():
    return {
        ...
        "adapters_loaded": engine.loaded_adapters,
        "queue": {"waiting": queue_waiting, "max": QUEUE_MAX, "busy": engine.lock.locked()},
...
async def adapters():
    return {
        "base_model": BASE_MODEL,
        "adapters": {
            name: {k: (str(v) if k == "path" else v) for k, v in info.items()} | {"loaded": name in engine.loaded_adapters, "provenance": provenance(name)}
            for name, info in ADAPTERS.items()
        },
...
async def generate(req: GenerateRequest, request: Request):
    ...
    if req.adapter != "base" and req.adapter not in engine.loaded_adapters:
        raise HTTPException(404, f"adapter '{req.adapter}' is not loaded")
    with queue_lock:
    ...
    t0 = time.time()
    try:
        stats = await asyncio.to_thread(engine.generate, req.prompt, req.duration, req.cfg, req.seed, req.adapter, out)
        pcs = await asyncio.to_thread(engine.pcs, out) if req.score else None
    finally:
        with queue_lock:
            queue_waiting -= 1
    meta = {
...
async def pcs(generation_id: str | None = None, file: UploadFile | None = File(default=None)):
    ...
    with queue_lock:
        ...
        queue_waiting += 1
    try:
        result = await asyncio.to_thread(engine.pcs, path)
